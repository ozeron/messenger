...
class APISession(object):
    ...
    def __call__(self, method_name, **method_kwargs):
        ...
        errors = []
        error_codes = []
        #print(response['captcha_sid'])
        full_error = json.JSONDecoder().decode(response.text)
        for data in json_iter_parse(response.text):
            if 'error' in data:
                error_data = data['error']
                if error_data['error_code'] == CAPTCHA_IS_NEEDED:
                    return self.captcha_is_needed(full_error['error'], method_name, **method_kwargs)

                error_codes.append(error_data['error_code'])
                errors.append(full_error['error'])
...
class VkAPIMethodError(VkError):
    ...
    def get_capthca_sid(self):
        return str(self.error['captcha_sid'])
    def get_capthca_url(self):
        return str(self.error['captcha_img'])
    def __str__(self):
