...
from PyQt5.QtCore import QDateTime, QVariant, QTimer, Qt, QObject
from PyQt5.uic import loadUi
from PyQt5.QtWidgets import QTableWidget
from PyQt5.QtGui import QBrush, QColor
from messenger import logger, groups_manager

class SelectGroupsDialog(QObject):
  ...
  def __init__(self, vk_client, ui):
    ...
    self.ui = ui
    self.grid = self.ui.gsGroupsTable
  ...
  def __init_connections(self):
    ...
    self.grid.verticalScrollBar().valueChanged.connect(self.__on_groups_slider_move)
    self.ui.editQuery.textChanged.connect(self.__on_query_text_changed)
    self.grid.cellDoubleClicked.connect(self.__on_double_click_on_group)
  ...
  def __populate_groups_list(self, count):
    ...
    groups = self.vk_client.vk_messenger.get_top_n_groups_by_location(cId, count, search_query, self.grid.rowCount())
